<template>
    <div class="flex flex-col py-4 px-6 shadow-md bg-white hover:bg-gray-50 ">
        <img :src="survey.image_url" alt="" class="w-full h-48 object-cover" style="height:54%" />
        ...
        <div class="flex justify-between items-center mt-3">
            ...
            <div class="flex items-center">
                <a :href="`/view/survey/${survey.slug}`" target="_blank" class="h-8 w-8 flex items-center justify-center rounded-full
                ...
                <button v-if="survey.id" type="button" @click="emit('delete', survey)"
                    class="h-8 w-8 flex items-center justify-center rounded-full border border text-sm text-red-500 focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
